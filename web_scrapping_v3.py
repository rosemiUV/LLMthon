...
async def procesar_web(page, url):
    ...
    resultado = {
        "status": None,      # Código HTTP (200, 404...)
        "is_junk": False,    # True si es web basura
        "text_content": "",  # Texto acumulado
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        for index, row in df.iterrows():
            ...
            # Construimos el JSON final fusionando datos
            datos_finales = {
                "id_excel": index,
                "empresa": nombre,
                "url": url_final,
                "status": resultado_web["status"],
                "is_junk": resultado_web["is_junk"],
                "text_content": resultado_web["text_content"],
                "error_msg": resultado_web["error_msg"]
            }
